...
## Text-Anchored Anomaly Detection (Extension)

> **⚠️ Text Description Declaration:** The anomaly detection module (`trainers/AnomalyDetect/`) uses **pre-generated text descriptions** as semantic anchors for the localization branch.

### Text Description Sources

All natural language descriptions in `trainers/AnomalyDetect/text_descriptions.py` are **offline pre-generated**, NOT generated during training or inference:

| Component | Source | Generation Method |
|-----------|--------|-------------------|
| Normal descriptions | Medical imaging report templates | Manually curated from domain knowledge |
| Anomaly descriptions | Medical imaging report templates | Manually curated from domain knowledge |

### Purpose

These descriptions are encoded **once** by the frozen BiomedCLIP text encoder to produce P_norm and P_anom anchor vectors, and remain **fixed** throughout training. They serve as the semantic "compass" that guides the patch-level anomaly localization without requiring pixel-level annotations.

### Adding New Datasets

To add a new dataset, register its text descriptions in `TEXT_DESCRIPTIONS_REGISTRY` within `trainers/AnomalyDetect/text_descriptions.py`:

```python
TEXT_DESCRIPTIONS_REGISTRY = {
    "BUSI": {
        "normal": [...],   # 3-5 normal descriptions
        "anomaly": [...],  # 3-5 anomaly descriptions
    },
    "YOUR_DATASET": {
        "normal": [...],
        "anomaly": [...],
...
For production use, replace the manual descriptions with **LLM-generated descriptions** (e.g., GPT-4) following the same format — the interface remains unchanged.
...
```bash
conda create -n anomaly_clip python=3.10 -y && conda activate anomaly_clip
pip install torch torchvision open_clip_torch timm yacs tqdm scikit-learn matplotlib
...
Download from [Kaggle](https://www.kaggle.com/datasets/aryashah2k/breast-ultrasound-images-dataset) (free account required), or use the kaggle CLI:
...
```
data/BUSI/BUSI/
  ├── benign_tumor/    (437 images)
  ├── malignant_tumor/ (210 images)
  └── normal_scan/     (133 images)
```
...
python train.py \
  ...
  --config-file configs/trainers/AnomalyDetect/few_shot/busi.yaml \
  --output-dir output/anomaly_detect \
  --seed 1
```
...
python visualize_heatmap.py \
  --image data/BUSI/BUSI/malignant_tumor/malignant\ (1).png \
  --checkpoint output/anomaly_detect/anomaly_clip/model.pth.tar-30 \
  --output result.png
```

Output: 3-panel figure (original + overlay + raw heatmap) with predicted class and anomaly score `s_img`.

### Anomaly Detection Files

| File | Description |
|------|-------------|
| `trainers/AnomalyDetect/anomaly_detect.py` | AnomalyCLIP model (two-path) + Trainer |
| `trainers/AnomalyDetect/text_descriptions.py` | Pre-generated medical text descriptions |
| `configs/trainers/AnomalyDetect/few_shot/busi.yaml` | Training hyperparameters |
| `visualize_heatmap.py` | Inference + heatmap visualization script |
